...
fn addCmakeCfgOptionsToExe(
    ...
    // llvm/Config/config.h is generated by LLVM's own build but, unlike
    // clang/Config/config.h, is never installed to the LLVM install prefix
    // (cfg.llvm_include_dir) — it's considered purely internal. Our
    // vendored llvm-objdump sources include it directly, so also point at
    // the sibling LLVM build tree's generated headers. cfg.llvm_include_dir
    // is "<out>/<target>-<mcpu>/include"; the build tree that generated it
    // lives at "<out>/build-llvm-<target>-<mcpu>/include" (see zig-bootstrap's
    // `build` script).
    if (std.fs.path.dirname(cfg.llvm_include_dir)) |install_prefix| {
        if (std.fs.path.dirname(install_prefix)) |out_dir| {
            const target_mcpu = std.fs.path.basename(install_prefix);
            const llvm_build_include = b.fmt("{s}/build-llvm-{s}/include", .{ out_dir, target_mcpu });
            mod.addIncludePath(.{ .cwd_relative = llvm_build_include });
        }
    }
...
fn addStaticLlvmOptionsToModule(b: *std.Build, mod: *std.Build.Module, options: struct {
    ...
    // llvm/Config/config.h is generated by LLVM's own build but, unlike
    // clang/Config/config.h, is never installed to a --search-prefix
    // (it's considered purely internal). Our vendored llvm-objdump sources
    // include it directly, so for every --search-prefix passed to this
    // build (e.g. "<out>/native-linux-gnu-baseline"), also point at the
    // sibling LLVM build tree that generated it: "<out>/build-llvm-native-
    // linux-gnu-baseline/include" (see zig-bootstrap's `build` script).
    for (b.search_prefixes.items) |prefix| {
        if (std.fs.path.dirname(prefix)) |out_dir| {
            const target_mcpu = std.fs.path.basename(prefix);
            const llvm_build_include = b.fmt("{s}/build-llvm-{s}/include", .{ out_dir, target_mcpu });
            mod.addIncludePath(.{ .cwd_relative = llvm_build_include });
        }
    }
